...
public class Room102 extends JPanel {
    private final MainFrame mainFrame;
    private boolean isShovelTaken = false; // 삽을 이미 가져갔는지 여부

    public Room102(MainFrame mainFrame) {
        ...
        JLabel locationLabel = new JLabel("병실 102호", SwingConstants.RIGHT);
        ...
        // 문 클릭 이벤트: 바로 복도로 이동
        roomPanel.addClickableArea(50, 70, 300, 430, "문 클릭됨", () -> mainFrame.switchToPanel(new FirstFloorHallway(mainFrame)));

        // 이불 클릭 이벤트
        roomPanel.addClickableArea(690, 360, 210, 40, "비어있다...", () -> showSimpleMessage("비어있다..."));

        // 상단 서랍 클릭 이벤트
        roomPanel.addClickableArea(390, 320, 120, 70, "장난감 삽이 들어있다.\n가져가시겠습니까?", this::handleShovelPickup);
    ...
    /**
     * 상단 서랍 클릭 시 삽을 가져가는 처리
     */
    private void handleShovelPickup() {
        if (isShovelTaken) {
        ...
        if (result == JOptionPane.YES_OPTION) {
            isShovelTaken = true; // 삽을 가져간 상태로 표시
            mainFrame.getInventory().addItem("장난감 삽"); // 아이템 창에 삽 추가
            mainFrame.getInventory().updateItems(); // 아이템 창 갱신
            showSimpleMessage("삽을 가져갔습니다.");
        }
    }
