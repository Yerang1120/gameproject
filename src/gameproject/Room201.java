...
public class Room201 extends JPanel {
    private final MainFrame mainFrame;
    private boolean isStickTaken = false; // 막대기 획득 여부 추적

    public Room201(MainFrame mainFrame) {
        ...
        JLabel locationLabel = new JLabel("병실 201호", SwingConstants.RIGHT);
        ...
        // 문 클릭 이벤트: 바로 복도로 이동
        roomPanel.addClickableArea(50, 70, 300, 430, "문 클릭됨", () -> mainFrame.switchToPanel(new SecondFloorHallway(mainFrame)));

        // 이불 클릭 이벤트
        roomPanel.addClickableArea(690, 360, 210, 40, "비어있다...", () -> showSimpleMessage("비어있다..."));

        // 상단 서랍 클릭 이벤트
        roomPanel.addClickableArea(390, 320, 120, 70, "막대기가 있다.\n가져가시겠습니까?", this::handleStickPickup);
    ...
    /**
     * 상단 서랍 클릭 시 막대기 획득 처리
     */
    private void handleStickPickup() {
        if (isStickTaken) {
        ...
        if (result == JOptionPane.YES_OPTION) {
            isStickTaken = true; // 막대기 상태 업데이트
            mainFrame.getInventory().addItem("막대기"); // 아이템 창에 막대기 추가
            mainFrame.getInventory().updateItems(); // 아이템 창 갱신
            showSimpleMessage("막대기를 가져갔습니다.");
        }
    }
