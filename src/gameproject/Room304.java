...
public class Room304 extends JPanel {
    private final MainFrame mainFrame;
    private boolean isNoteTaken = false; // 쪽지를 이미 가져갔는지 여부

    public Room304(MainFrame mainFrame) {
        ...
        JLabel locationLabel = new JLabel("병실 304호", SwingConstants.RIGHT);
        ...
        // 문 클릭 이벤트: 바로 복도로 이동
        roomPanel.addClickableArea(50, 70, 300, 430, "문 클릭됨", () -> mainFrame.switchToPanel(new ThirdFloorHallway(mainFrame)));
        ...
        // 하단 서랍 클릭 이벤트
        roomPanel.addClickableArea(390, 410, 120, 70, "쪽지가 들어있다.\n가져가시겠습니까?", this::handleNotePickup);
    }

    /**
     * 하단 서랍 클릭 시 쪽지를 가져가는 처리
     */
    private void handleNotePickup() {
        ...
        if (result == JOptionPane.YES_OPTION) {
            isNoteTaken = true; // 쪽지를 가져간 상태로 표시
            mainFrame.getInventory().addItem("쪽지: 2847"); // 아이템 창에 쪽지 추가
            mainFrame.getInventory().updateItems(); // 아이템 창 갱신
            showSimpleMessage("쪽지를 가져갔습니다.");
        }
    }
